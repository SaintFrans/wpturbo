...
app/
  ...
  Concerns/         Organizations/ (HasOrganizations, GeneratesHandle) + GeneratesPublicId + validation-rule traits
  ...
  Http/
    Controllers/    Settings/  Organizations/ (incl. AuditLogController)  Clients/
    Middleware/     EnsureOrganizationMembership, SetOrganizationUrlDefaults, HandleInertiaRequests
    Requests/       Settings/  Organizations/  Clients/
    Responses/      Fortify response overrides (post-login redirect targets)
  Models/           User + Organizations/ (Organization, Membership, OrganizationInvitation)  Audit/ (AuditLogEntry)  Clients/ (Client)
  Notifications/    Organizations/
  Policies/         Organizations/ (OrganizationPolicy)  Clients/ (ClientPolicy)
...
```
resources/js/
  pages/        Inertia page components (auth/, settings/, organizations/settings/, clients/, dashboard, welcome)
  layouts/      flat — app, content, section, auth, settings (ADR-042, ADR-045, ADR-046)
  components/   shell/, organizations/, clients/, account/ + ui/ (shadcn primitives); the root holds
                only what every domain uses (ADR-045)
  hooks/        appearance, clipboard, two-factor, mobile detection
  types/        shared TS types, incl. organizations.ts and clients.ts mirroring the PHP DTOs
...
`organizations/settings/*` and `settings/*` both get `SettingsLayout`: one Settings sidebar with an
Account group and the current organization's beneath it (ADR-046). The two URL spaces stay separate
— `/settings/…` is the person's and `/org/{organization}/settings/…` the tenant's, each with its own
middleware — only the navigation is merged.
...
| Scope         | Example                                                                                        |
| ------------- | ---------------------------------------------------------------------------------------------- |
| Tenant-scoped | `/org/{organization}/dashboard`, `/org/{organization}/clients`, `/org/{organization}/settings` |
| Tenant root   | `/org/{organization}` redirects to that organization's Overview — never a 404                  |
| Personal      | `/settings/profile`, `/settings/security`                                                      |
| Neither       | `/org` (redirects into your own organization), `/invitations/{code}/accept`                    |

`/org/{organization}/clients` is the first resource route to exist. Every future tenant resource —
servers, sites, domains — goes under `/org/{organization}/…` the same way, inside
...
`/org/{organization}` itself (`organizations.home`) redirects to the dashboard. A tenant prefix is
something people type, shorten and bookmark, so the bare prefix resolves rather than 404s. It sits
**inside** `EnsureOrganizationMembership` on purpose: a non-member gets the same 403 as on any other
route under the prefix, so the redirect cannot be used to probe which handles exist.
