...
**Status: open, deliberately parked.** The Cyberbeveiligingswet took effect on 15 August 2026 and
names managed service providers and hosting providers explicitly. Whether it applies here is
**not settled**: the platform manages servers the customer owns rather than supplying its own, and
whether that makes it a managed service provider in the sense the law means is a question for
someone qualified to answer it. Do not treat this section as a compliance position.
...
those is a coding task and none is done. **Detection is the one that is** — see G12 — and it is
deliberately not a priority right now.
...
| G4  | Organization soft-delete hard-deletes memberships                                          | A restored organization is ownerless and unreachable                                                                                                                                                                                                                            | [ADR-019](DECISIONS.md), [ADR-034](DECISIONS.md) |
...
| G8  | Production password policy is environment-conditional                                      | If `APP_ENV` is ever wrong in production, the policy silently disappears                                                                                                                                                                                                        | See below                                        |
...
this order: G2 and G4 (small), G6 (small, one migration), G5 (the largest), G3 (needs the database
constraint and the transfer flow from [ADR-020](DECISIONS.md)).

**G8 needs no ADR.** The production password policy is keyed on `APP_ENV`, so a misconfigured
environment silently removes it. The fix is to invert the condition — apply the policy everywhere
except in the test environment — so that getting the environment wrong makes local development
stricter rather than production weaker. Failing safe rather than failing open, and it costs
nothing.

**G7 does not exist.** The number was skipped when the table was first written; nothing is
missing.
